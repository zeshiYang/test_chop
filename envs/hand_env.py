...
class HandEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, model_path, openloop_path, mode='single', render_mode=None, debug=False):
        ...
        self.frame_skip = 1
        self.pose_parameter = False
        self.num_steps = 0
        self.num_steps_sim = 0
        self.time_step = 0.01
        self.object_dof = 7 + 7
        self.mocap_dof = 8
        self.body_dof = 0 # the dof of the upper human body
        self.perturb_prob = 0
        self.rsi = True
        ...
        self.action_space.high = self.sim.model.jnt_range[:self.dof_control,1]
        self.action_space.low  = self.sim.model.jnt_range[:self.dof_control,0]
    ...
    def _get_chopsticks_drdq(self):
        ...
        quat_chopstick1 = convert2glm(self.sim.data.body_xquat[self.body_chopstick1])
    ...
    def _get_rot_axis(self, t):
        ...
        quat_palm = convert2glm(self.sim.data.body_xquat[self.sim.model.body_name2id('palm')])
    ...
    def _compute_tippos(self, t):
        ...
        for i in range(6):
            if(chop_id[i] == 1):
                pos = convert2glm(q_chop1) * convertfromvec2glm(rel_pos[i]) * glm.inverse(convert2glm(q_chop1))
                pos = r_chop1 + convertfromglm2vec(pos)
            elif(chop_id[i] == 2):
                pos = convert2glm(q_chop2) * convertfromvec2glm(rel_pos[i]) * glm.inverse(convert2glm(q_chop2))
                pos = r_chop2 + convertfromglm2vec(pos)
            else:
                pos = np.array([0,0,0])
    ...
    def _chop_kin(self, t):
        ...
        quat_palm = convert2glm(self.sim_chopsticks.data.body_xquat[self.sim_chopsticks.model.body_name2id('palm')])
        pos_palm = convertfromvec2glm(self.sim_chopsticks.data.body_xpos[self.sim_chopsticks.model.body_name2id('palm')])
        ...
        return convertfromglm2vec(pos_chop2), convert2array(quat_chop2), convertfromglm2vec(pos_chop1), convert2array(quat_chop1)
    ...
    def step(self, action):
        ...
        infos['dr'] = convertfromglm2vec(dr)
        infos['dq'] = convert2array(dq)
    ...
    def _get_obs(self):
        ...
        quat_chopstick_current_ref = convert2glm(pose_chopstick_current_ref[3:7])
        dr_object_ref = convertfromglm2vec( glm.inverse(quat_chopstick_current_ref) * convertfromvec2glm(pose_object_current_ref[0:3] - pose_chopstick_current_ref[0:3]) * ( quat_chopstick_current_ref))
        ...
        quat_chopstick_sim = convert2glm(quat_chopstick2_sim)
        dr_object_sim = convertfromglm2vec( glm.inverse(quat_chopstick_sim) * convertfromvec2glm(pos_object_sim - pos_chopstick2_sim)*( quat_chopstick_sim))
        dv_object_sim = convertfromglm2vec( glm.inverse(quat_chopstick_sim) * convertfromvec2glm(velp_object_sim - velp_chopstick2_sim)*( quat_chopstick_sim))
        feature_object = [dr_object_ref, dv_object_ref, dr_object_sim, dv_object_sim, geom_type, geom_size]
     
        quat_palm_sim = convert2glm(self.sim.data.body_xquat[self.body_palm])
        ...
        quat_chopstick2_sim = convert2glm(quat_chopstick2_sim)
        dq_chopstick2_sim = convert2array( glm.inverse(quat_palm_sim) * quat_chopstick2_sim )
        dr_chopstick2_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(pos_chopstick2_sim - pos_palm_sim) * quat_palm_sim )

        dq_chopstick2_ref = convert2array(self.dq_chopstick2)
        dr_chopstick2_ref = convertfromglm2vec(self.dr_chopstick2)

        diff_dq_chopstick2 = convert2array( glm.inverse(convert2glm(dq_chopstick2_sim)) * convert2glm(dq_chopstick2_ref))
        diff_dr_chopstick2 = dr_chopstick2_sim - dr_chopstick2_ref

        dvp_chopstick2_ref = np.array([0,0,0])
        dvr_chopstick2_ref = np.array([0,0,0])

        dvp_chopstick2_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(velp_chopstick2_sim - velp_palm_sim) * quat_palm_sim )
        dvr_chopstick2_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(velr_chopstick2_sim - velr_palm_sim) * quat_palm_sim )
        ...
        quat_chopstick1_sim = convert2glm(quat_chopstick1_sim)
        dq_chopstick1_sim = convert2array( glm.inverse(quat_palm_sim) * quat_chopstick1_sim )
        dr_chopstick1_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(pos_chopstick1_sim - pos_palm_sim) * quat_palm_sim )

        quat_palm_ref = convert2glm(quat2_ref) * glm.inverse(self.dq_chopstick2)
        pos_palm_ref = pos2_ref - convertfromglm2vec( quat_palm_ref * self.dr_chopstick2 * glm.inverse(quat_palm_ref))

        dq_chopstick1_ref = convert2array( self.dq_chopstick2 * glm.quat(np.cos(pose[-1]/2), 0, 0, np.sin(pose[-1]/2)) )
        dr_chopstick1_ref = convertfromglm2vec( glm.inverse(quat_palm_ref) * convertfromvec2glm(pos1_ref - pos_palm_ref) * quat_palm_ref )

        diff_dq_chopstick1 = convert2array( glm.inverse(convert2glm(dq_chopstick1_sim)) * convert2glm(dq_chopstick1_ref))
        diff_dr_chopstick1 = dr_chopstick1_sim - dr_chopstick1_ref

        dvp_chopstick1_ref = convertfromglm2vec( glm.inverse(quat_palm_ref) * convertfromvec2glm(vp1_ref - vp2_ref) * quat_palm_ref )
        dvr_chopstick1_ref = convertfromglm2vec( glm.inverse(quat_palm_ref) * convertfromvec2glm(vr1_ref - vr2_ref) * quat_palm_ref )

        dvp_chopstick1_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(velp_chopstick1_sim - velp_palm_sim) * quat_palm_sim )
        dvr_chopstick1_sim = convertfromglm2vec( glm.inverse(quat_palm_sim) * convertfromvec2glm(velr_chopstick1_sim - velr_palm_sim) * quat_palm_sim )
    ...
    def _reset_sim(self):
        ...
        #compute pose parameters: pos and rotation
        if(self.pose_parameter==False):
            self.pose_parameter = True
    ...
    def get_num_task(self):
        if(self.mode == "single"):
            ...
            return len(self.openloop_generator_list)
