...
class TrajectoryPlanner(object):
    ...
    def opt_move_seq(self, q_init = None, start_transformation = np.array([0,0,0,1,0,0,0]), end_transformation=np.array([0,0,0,1,0,0,0]), object_idx = 0, dr = None, dq = None, mode = 'pregrasp'):
        ...
        t_rotate = glm.inverse(convert2glm(start_transformation[3:7])) * convert2glm(end_transformation[3:7])
        if(t_rotate[3]<0):
            t_rotate = -t_rotate
        ...
        def f(x, compute_dist = 1):
            ...
            for i in range(int(np.floor(t/dt)) + 1):
                ...
                q = glm.slerp(convert2glm(start_transformation[3:7]), convert2glm(end_transformation[3:7]), beta.cdf(i*dt/t, beta_para1, beta_para2))
        ...
        for i in range(motions.shape[0]):
            ...
            q = glm.slerp(convert2glm(start_transformation[3:7]), convert2glm(end_transformation[3:7]), beta.cdf(i*0.01/t, beta_para1, beta_para2))
            if(end_transformation[3] * q[3] <0):
                q = -q
            theta = start_transformation[-1] + (end_transformation[-1] - start_transformation[-1]) * (i*0.01/t)
            pose = np.concatenate([pos,convert2array(q), np.array([theta])])
            motions[i, 1:] = pose
        return motions
