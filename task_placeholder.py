    ...
    if(type == 'motion'):
        ...
        if(motion.shape[1] == 9):
            for i in range(vel.shape[0]-1):
                ...
                vel[i, 1+3: 1 + 3 + 3] = compute_anglevel(convert2glm(motion[i, 1+3: 1 +3 + 4]), convert2glm(motion[i+1, 1+3 : 1+3+4]), motion[0,0])
            ...
            for i in range(vel.shape[0]-1):
                vel[i, 1: 1+ 3] = (motion[i+1, 1:1+3] - motion[i, 1:1+3])/motion[0,0]
                vel[i, 1+3:] = compute_anglevel(convert2glm(motion[i, 1+3:]), convert2glm(motion[i+1, 1+3 :]), motion[0,0])
            vel[-1,1:] = vel[-2,1:].copy()
        else:
...
def grasp_task_holder(chop_xml, pose_file, transform_chopsticks_start, objects_paras, transform_objects_start, transform_objects_end, thumb_relative_position = 0, save_folder= None, task_path = None, plate_sampler = None):
    ...
    def computedrdq(transform_object, transform_chopsticks):
        ...
        quat_chopstick2 = convert2glm(transform_chopsticks[3:7])
        quat_object = convert2glm(transform_object[3:])
    ...
    def compute_transform_chopsticks_from_object(transform_object, dr, dq, theta):
        ...
        '''
        quat_chopsticks = convert2glm(transform_object[3:7]) * glm.inverse(dq)
        pos_chopsticks = transform_object[0:3] - convertfromglm2vec(quat_chopsticks * dr * glm.inverse(quat_chopsticks))
        return np.concatenate([pos_chopsticks, convert2array(quat_chopsticks), np.array([theta])]) 
    ...
    def compute_transform_object_from_chopsticks(transform_chopsticks, dr, dq):
        ...
        '''
        q = convert2array(convert2glm(transform_chopsticks[3:7]) * dq)
        pos = transform_chopsticks[0:3] + convertfromglm2vec(convert2glm(transform_chopsticks[3:7]) * dr * glm.inverse(convert2glm(transform_chopsticks[3:7])))
    ...
    if(objects_paras!=None):
        ...
        for i in range(num_objects):
            ...
            if(geom == 'capsule'):
                ...
                quat = convert2array(glm.quat(np.cos(theta/2), 0, 0, np.sin(theta/2)))
